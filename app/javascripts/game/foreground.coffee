...
define 'game.Foreground', ->
  ...
  Foreground = meta.def 'game.Foreground',
    ...
    init: (@map, @width, @height) ->
      @objects = require('game.CollidableMatrix').create(this)
      @framedObjects = @objects.clone().extend(require('game.FramedObjectMatrix'))
      @blocks = []
      @player = null
    ...
    getObjectsWithout: (object) ->
      coll =
        if @enableCollisions
          ...
          require('game.CollidableMatrix').create(this)
