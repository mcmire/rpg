...
define 'game.LiveObject', ->
  meta = require('meta')
  StillObject = require('game.StillObject')

  # A LiveObject represents an object on the map that lives in the foreground
  # layer and is thus collidable. In addition, it can also be programmed to be
  # in different states, such as "alive" or "dead" or "moving left". Finally, a
  # LiveObject has a sprite for an image, which is to say the image is composed
  # of frames, and each state is associated with a slice of those frames. If a
  # slice has only one frame then the object is drawn to have a static image;
  # otherwise, the frames are animated. This animation can either play
  # repeatedly, or stop after it is complete.
  #
