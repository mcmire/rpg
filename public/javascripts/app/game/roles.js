(function() {
  ...
  (game = this.game).define('roles', function(name) {
    var ROLES, assignable, attachable, drawable, eventHelpers, eventable, loadable, runnable, simpleDrawable, tickable, _getSafeNameFrom;
    ...
    eventable = this.meta.def('game.eventable', {
    ...
    attachable = this.meta.def('game.attachable', {
      destroy: function() {
        this.detach();
        return this._super();
      },
    ...
    tickable = this.meta.def('game.tickable', {
      tick: function() {
        throw new Error('tick must be overridden');
      }
    });
    simpleDrawable = this.meta.def('game.simpleDrawable', {
      draw: function() {
        throw new Error('draw must be overridden');
      }
    });
    drawable = this.meta.def('game.drawable', tickable, simpleDrawable, {
      tick: function(ctx) {
        this.predraw(ctx);
        this.draw(ctx);
        this.postdraw(ctx);
    ...
    loadable = this.meta.def('game.loadable', {
      init: function() {
        var args;
    ...
    runnable = this.meta.def('game.runnable', {
      destroy: function() {
        this.stop();
        return this._super();
      },
    ...
    assignable = this.meta.def('game.assignable', {
