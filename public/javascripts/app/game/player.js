(function() {
  ...
  (game = this.game).define('player', function(name) {
    var DIRECTIONS, DIRECTION_KEYS, KEYS, KEY_DIRECTIONS, dir, keyCode, player, _i, _j, _len, _len2, _ref;
    DIRECTIONS = 'up down left right'.split(' ');
    DIRECTION_KEYS = {
      up: this.keyboard.keyCodesFor('KEY_W', 'KEY_UP'),
      down: this.keyboard.keyCodesFor('KEY_S', 'KEY_DOWN'),
      left: this.keyboard.keyCodesFor('KEY_A', 'KEY_LEFT'),
      right: this.keyboard.keyCodesFor('KEY_D', 'KEY_RIGHT')
    };
    KEY_DIRECTIONS = {};
    ...
    player = this.LiveObject.cloneAs(name);
    player.extend(this.roles.eventable, {
      viewportPadding: 30,
      keyTracker: this.keyboard.KeyTracker.create(KEYS),
      addEvents: function() {
        return game.keyboard.addKeyTracker(this.keyTracker);
      },
      removeEvents: function() {
        return game.keyboard.removeKeyTracker(this.keyTracker);
      },
      activate: function() {
